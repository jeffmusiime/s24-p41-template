...
object VerticesEdgesCounts {
  ...
  def verticesAndNodesCount(inputPath: String, spark: SparkSession, sc: SparkContext): String = {
    ...
    val graphRDD = sc.textFile(inputPath)
      .map(_.split("\t")) // Split lines into (source, destination) pairs

    val verticesRDD = graphRDD.flatMap(fields => Seq(fields(0), fields(1)))
    val distinctVertices = verticesRDD.distinct().count()

    val distinctEdges = graphRDD.distinct().count() // Avoid duplicate counting for directed edges

    countsToString(distinctVertices, distinctEdges)
